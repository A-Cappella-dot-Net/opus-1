...
public class TcpProxy extends Thread {
	...
	public TcpProxy(InetAddress srcAddr, int srcPort, InetAddress dstAddr,
		...
		this.header = (srcAddr == null ? "" : srcAddr.toString()) + ":"
		...
		try {
			while (!isInterrupted()) {
				...
				try {
					...
					StreamCopyThread sToC = new StreamCopyThread(serverSocket,
							clientSocket);
					StreamCopyThread cToS = new StreamCopyThread(clientSocket,
							serverSocket);
					sToC.setPeer(cToS);
					cToS.setPeer(sToC);
					synchronized (lock) {
	...
	private static TcpProxy addProxy(String src, String srcPort, String dst,
			String dstPort, PrintStream out, PrintStream err)
			throws UnknownHostException, IOException {
		InetAddress srcAddr = (src == null) ? null : InetAddress.getByName(src);
		return new TcpProxy(srcAddr, Integer.parseInt(srcPort),
	...
	private static void parseConfigFile(String fileName,
		...
		in.close();
		for (int i = 0;; i++) {
			String srcAddr = props.getProperty(propertyPrefix + "." + i
					+ ".sourceAddr");
			String srcPort = props.getProperty(propertyPrefix + "." + i
					+ ".sourcePort");
			if (srcPort == null)
				break;
			String dstAddr = props.getProperty(propertyPrefix + "." + i
					+ ".destAddr");
			String dstPort = props.getProperty(propertyPrefix + "." + i
					+ ".destPort");
			if (dstAddr == null) {
				throw new IllegalArgumentException(
						"Missing destination address for proxy " + i);
			}
			if (dstPort == null) {
				throw new IllegalArgumentException(
						"Missing destination port for proxy " + i);
			}
			addProxy(srcAddr, srcPort, dstAddr, dstPort, out, err);
		}
	}

	static void parseArguments(String[] argv, PrintStream out,
			PrintStream err) throws FileNotFoundException, IOException,
		...
		if (argv.length > 1) {
			...
			addProxy(src, argv[argBase++], argv[argBase++], argv[argBase++], out, err);
		} else if (argv.length == 1) {
			parseConfigFile(argv[0], out, err);
		} else {
